...
from cc_spec.rag.incremental import GitChangeSet
...
class TestCountChangedFiles:
    """测试 _count_changed_files 函数。"""

    @patch("cc_spec.commands.quick_delta.detect_git_changes")
    def test_count_changed_files_filters_cc_spec(self, mock_detect):
        """应过滤 .cc-spec 目录下的变更。"""
        mock_detect.return_value = GitChangeSet(
            changed=["src/main.py", ".cc-spec/config.yaml"],
            removed=["docs/old.md"],
            untracked=[".cc-spec/changes/tmp.txt", "README.md"],
        )

        result = _count_changed_files(Path("/repo"))

        # 仅统计非 .cc-spec 路径
        assert result == 3

    @patch("cc_spec.commands.quick_delta.detect_git_changes")
    def test_count_changed_files_no_git(self, mock_detect):
        """无 git 环境返回 None。"""
