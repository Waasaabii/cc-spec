...
class TestQuickDeltaCommand:
    ...
    def test_quick_delta_without_project_root(
        self,
        tmp_path_factory: pytest.TempPathFactory,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Test quick-delta command fails when not in a project."""
        isolated_dir = tmp_path_factory.mktemp("isolated-quick-delta")
    ...
    def test_quick_delta_creates_archive_directory(self) -> None:
        ...
        assert_contains_any(result.stdout, ["成功", "Quick-delta record created successfully"])
    ...
    def test_quick_delta_shows_preview(self) -> None:
        ...
        assert_contains_any(result.stdout.lower(), [message.lower().replace(" ", "-"), message.lower()])
        assert_contains_any(result.stdout.lower(), ["已创建", "created", "变更名称"])
    ...
    def test_quick_delta_shows_archived_path(self) -> None:
        ...
        assert result.exit_code == 0
        # Support Chinese and English output
        assert_contains_any(result.stdout, ["归档于", "Archived to:", ".cc-spec"])
        # Handle both Windows and Unix path separators
        assert_contains_any(result.stdout, [".cc-spec", "cc-spec"])
