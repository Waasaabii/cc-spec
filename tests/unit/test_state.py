...
def temp_state_file(tmp_path: Path) -> Path:
    ...
    write_yaml(state_file, state_data)
...
def temp_cc_spec_root(tmp_path: Path) -> Path:
    ...
    write_yaml(change1_dir / "status.yaml", state1)
    ...
    write_yaml(change2_dir / "status.yaml", state2)
    ...
    write_yaml(archived_change / "status.yaml", state3)
...
class TestLoadState:
    ...
    def test_load_state_minimal(self, tmp_path: Path) -> None:
        ...
        write_yaml(minimal_file, {"change_name": "test"})
