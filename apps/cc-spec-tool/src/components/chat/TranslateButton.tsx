    ...
    return (
        <div className="flex flex-col items-start gap-1">
            ...
            {showTranslated && translated && (
                <div className={`text-[11px] leading-relaxed rounded px-2 py-1 ${theme === "dark" ? "bg-slate-800 text-slate-200 border border-slate-700" : "bg-slate-100 text-slate-700 border border-slate-200"}`}>
                    {translated}
                </div>
            )}

            {error && (
                <span className={`text-[10px] ${theme === "dark" ? "text-rose-400" : "text-rose-600"}`}>
