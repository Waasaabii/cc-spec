...
export function RunCard({ run, lang, t, theme, sessions, isCompact = false }: RunCardProps) {
    ...
    const canLaunchTerminal = Boolean(run.projectRoot);
                ...
                <div className="flex items-center gap-2">
                    ...
                    {run.projectRoot && (
                        <button
                            ...
                            disabled={!canLaunchTerminal || isLaunching}
                            className={`transition-colors p-1 rounded ${theme === "dark" ? "text-slate-400 hover:text-slate-200 hover:bg-slate-700/50" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/60"} ${!canLaunchTerminal || isLaunching ? "opacity-50 cursor-not-allowed hover:bg-transparent" : ""}`}
                            title={lang === "zh" ? "打开 Claude 终端" : "Open Claude terminal"}
                            aria-label={lang === "zh" ? "打开 Claude 终端" : "Open Claude terminal"}
                        >
                            <Icons.Terminal />
                        </button>
                    )}
                                        ...
                                        return (
                                            <div key={idx} className={theme === "dark" ? "text-slate-200" : "text-slate-800"}>
                                                <div className="flex items-start gap-2">
                                                    <span className={`mt-0.5 ${theme === "dark" ? "text-slate-500" : "text-slate-400"}`}>?</span>
                                                    <div className="flex-1 min-w-0">
                                                        <RenderContent content={line.content} type={line.type} theme={theme} />
                                                        <div className="mt-1">
                                                            <TranslateButton text={line.content} theme={theme} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
